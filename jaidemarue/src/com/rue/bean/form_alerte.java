...
public final class form_alerte {
    ...
    private static final String CHAMP_COORD = "coordAlerte";
    ...
    public Alerte creerAlerte( HttpServletRequest request ) {
     ...
     String coord = getValeurChamp( request, CHAMP_COORD);

        Alerte alerte = new Alerte();
        
        alerte.setOwner(user);
...
throw new Exception( "Merci d'entrer un type." );
    ...
    private void validationImage( String image ) throws Exception {
        if ( image != null ) {
            if ( image.length() < 2 ) {
                throw new Exception( "L'image de l'alerte doit contenir au moins 2 caracteres." );
            }
        } else {
